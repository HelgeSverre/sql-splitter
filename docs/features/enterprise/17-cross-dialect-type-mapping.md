...
plan schema v23, PostgreSQL catalog format v6, MySQL catalog format v6.
...
- Both directions can finish only as
  `CompletedWithApprovedTransformations`. Plan schema v23 is the compatibility
...
MySQL `AUTO_INCREMENT` is supported only when the reviewed PostgreSQL target
type is `smallint`, `integer`, or `bigint`. The plan records a typed
`CrossDialectPostgresIdentity` contract with the source and target column,
the migration-owned sequence name, the exact PostgreSQL integer type, and the
frozen MySQL next value `N`.

The source catalog also binds the session and global
`auto_increment_increment` and `auto_increment_offset` values. All four values
must be exactly 1. A different or missing value blocks planning because the
first PostgreSQL subset uses an increment-1 identity and cannot reproduce a
different MySQL generation stride.

The target table is created without an identity default. The copy therefore
writes every reviewed source identifier explicitly. After the table copy is
durably complete, the executor adds `GENERATED BY DEFAULT AS IDENTITY` with a
typed, quoted `ALTER TABLE` statement and sets the owned sequence to
`setval(sequence, N, false)`. The next target insert that omits the identifier
receives exactly `N`. Resume inspects the complete column, sequence,
ownership, configuration, and `(last_value, is_called)` state. Exact state is
accepted, absence is retried, and any third state requires manual
reconciliation.

This is a reviewed behavioral transformation, not complete future-DML
equivalence. PostgreSQL generates on an omitted identifier or `DEFAULT`, but
not on an explicit `NULL`. An explicit identifier is accepted by `BY DEFAULT`,
but it does not advance the PostgreSQL sequence as a MySQL explicit
AUTO_INCREMENT value can advance the table counter. The non-blocking
`cross_dialect_identity` finding states both differences for each affected
column. Applications that depend on either MySQL behavior need a separately
reviewed compatibility layer and cannot treat this identity subset as exact.

The retained MySQL snapshot and external freeze bind `N` through the source
catalog fingerprint. A post-freeze insert, DDL change, or changed next value
invalidates the source consistency evidence before another target effect.

MySQL `BIGINT UNSIGNED AUTO_INCREMENT` remains blocking. Its lossless target
type is `numeric(20,0)`, but PostgreSQL identity columns require an integer
type and PostgreSQL sequences cannot represent the full unsigned 64-bit
generation range. The planner reports this before any target effect. Other
AUTO_INCREMENT columns whose next value is missing or outside the selected
PostgreSQL integer range also block planning.
...
- The recovery matrix passes on all six PostgreSQL 15/16/17 × MySQL 8.0/8.4
  ...
  `identity-prepared`, `identity-effect-applied`, `identity-committed`,
...
- The MySQL `BIGINT AUTO_INCREMENT` to PostgreSQL `bigint` identity baseline
  and the `identity-effect-applied` recovery boundary passed on PostgreSQL 17
  and MySQL 8.4 on 2026-08-15. The source table recorded next value 42. After
  resume, the target column was `GENERATED BY DEFAULT`, strict verification
  completed, and one insert that omitted the identifier returned 42. The
  remaining supported version pairs still require the same identity-specific
  live rerun before this increment has complete matrix evidence.
