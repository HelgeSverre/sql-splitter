...
- **`.zip` archive input** — every command that reads a dump (`split`, `merge`, `analyze`, `validate`, `diff`, `graph`, `order`, `convert`, `redact`, `query`, `sample`, `shard`) now accepts a `.zip` file containing exactly one `.sql` member, deflated or stored. Detection is a two-phase open: `zip::ZipArchive` parses the central directory on a seekable `File` to locate the member (handling zip64 and junk entries like `__MACOSX/`/`.DS_Store`/dotfiles for free), then a fresh `File` is reopened and seeked to the member's data and streamed through an ordinary decompressor — downstream parsing code never knows the input was zipped. Multiple/zero `.sql` members, encrypted members, and unsupported compression methods produce a clear error. Feature-gated under the existing `archive` Cargo feature; `--compress zip` for per-table *output* stays rejected (use `-o dump.zip` for archive output instead).
