...
- **Semantic-preserving ENUM conversion between PostgreSQL and MySQL** — `convert` now translates enums in both directions instead of collapsing them to strings. MySQL inline `ENUM(...)` columns become PostgreSQL `CREATE TYPE ... AS ENUM` definitions plus typed columns (the `CREATE TYPE` is emitted directly before the DDL that first uses it, including enums introduced by `ALTER TABLE ... MODIFY COLUMN`); PostgreSQL enum types become inline MySQL `ENUM(...)` at the type position only, leaving string literals, comments, and column names untouched. `ALTER TYPE ... ADD VALUE` (with `BEFORE`/`AFTER`, `IF [NOT] EXISTS`, and schema-qualified names) updates the registry so later tables see the full label list; labels containing parentheses, quotes, or MySQL backslash escapes survive intact. Non-enum `CREATE TYPE` forms (composite, range, `RENAME`) keep the skip-with-warning behavior. New flag `--enum-naming per-column|dedupe` controls generated type names (`enum__{table}__{column}`, or reuse across columns with identical labels). ENUM still narrows to `TEXT`/`NVARCHAR(255)` with a warning for SQLite/MSSQL targets.
