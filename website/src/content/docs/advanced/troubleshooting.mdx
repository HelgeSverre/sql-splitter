...
### Dialect detection fails on generic SQL

sql-splitter analyzes the first 8 KB of the file to detect the dialect. If your dump starts with generic SQL, detection falls back to MySQL with low confidence.
...
This shouldn't happen with normal sql-splitter commands (they use ~20–150 MB, independent of file size). If it does:
...
- **ENUM types** → Properly converted between PostgreSQL and MySQL (bidirectional, preserves semantics). Use `--enum-naming` to control the naming strategy for MySQL→PostgreSQL conversions. Warnings only fire for lossy conversions (ENUM → TEXT for SQLite, ENUM → NVARCHAR for MSSQL).
