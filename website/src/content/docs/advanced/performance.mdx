...
write profile. Measured on a 1TB USB HDD (124MB→3.8GB fixtures):

| Profile               | Throughput (same-spindle split) |
| --------------------- | ------------------------------- |
| `fast` (SSD defaults) | 22–43 MB/s                      |
| `hdd` (pinned)        | 55–77 MB/s (**1.5–2.5×**)       |
| `auto`                | matches or beats pinned `hdd`   |

Output is byte-identical in every profile. Pin with `--io-profile hdd` or
`minimal-ops` (very slow flash / network mounts) when you already know the
target device.
