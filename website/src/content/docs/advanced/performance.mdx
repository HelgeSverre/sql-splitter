...
| `cheap`       | 1                           | 32 MB chunks, coalesced up to 64 MB | 512 MB | Cheap USB flash and network filesystems: per-operation cost dominates regardless of access pattern, so issue the fewest, largest operations possible (aliases: `shit`, `potato` — we know what it is like out there) |
