...
target with `scripts/verify-io-profiles.sh`. Output is byte-identical in every
profile. Pin with `--io-profile hdd` or `minimal-ops` (very slow flash /
network mounts) when you already know the target device.
