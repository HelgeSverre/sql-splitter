...
- **ENUM conversion**: MySQL ↔ PostgreSQL `ENUM` is semantic-preserving (MySQL ENUM → PostgreSQL `CREATE TYPE` + typed column, PostgreSQL ENUM → MySQL inline `ENUM`). Lossy for SQLite (`ENUM` → `TEXT`) and MSSQL (`ENUM` → `NVARCHAR(255)`).
