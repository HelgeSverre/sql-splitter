...
- **ENUM conversion**: MySQL ↔ PostgreSQL `ENUM` is semantic-preserving (MySQL ENUM → PostgreSQL `CREATE TYPE` + typed column, PostgreSQL ENUM → MySQL inline `ENUM`). Lossy for SQLite (`ENUM` → `TEXT`) and MSSQL (`ENUM` → `NVARCHAR(255)`). Conversion streams statement-by-statement: an `ALTER TYPE ... ADD VALUE` updates only tables converted after it (standard `pg_dump` order defines types before tables, so typical dumps are unaffected).
