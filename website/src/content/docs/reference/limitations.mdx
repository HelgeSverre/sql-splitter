...
- **A `;` inside a backtick-quoted identifier or inside a `/* */` block comment splits the statement early.** Quoted _strings_ are handled correctly; backtick identifiers and mid-statement block comments are not.
...
- Both `GO` _and_ `;` terminate statements, so multi-statement batches (procedure bodies, `BEGIN ... END` blocks with internal semicolons) are split apart.
...
- **Skipped**: session commands (`SET NAMES`, `SET ANSI_NULLS`, `LOCK TABLES`, `PRAGMA`, ...), and PostgreSQL-only DDL (`CREATE FUNCTION`/`DOMAIN`/`SEQUENCE`/`EXTENSION`/`TRIGGER`, non-enum `CREATE TYPE`/`ALTER TYPE` forms, `COMMENT ON`, `GRANT`) — each with a warning.
- **Rewritten lossily**: MySQL `SET(...)` becomes `TEXT` (constraint semantics lost); `UNSIGNED` and `ZEROFILL` are removed; inline `COMMENT` clauses are stripped for PostgreSQL targets.
- **ENUM conversion**: MySQL ↔ PostgreSQL `ENUM` is semantic-preserving for declarations that appear before their tables (the order produced by `pg_dump`). A MySQL label containing NUL cannot be represented by PostgreSQL, so its complete DDL statement is skipped with a warning or rejected in strict mode. PostgreSQL enum arrays narrow to MySQL `JSON` with a warning. SQLite uses `TEXT`; MSSQL uses `NVARCHAR(255)`. Because conversion streams statement-by-statement, `ALTER TYPE` after a table cannot rewrite that already-emitted table and reports a warning. Cross-dialect `ALTER COLUMN TYPE`/`MODIFY COLUMN` can require manual restoration of column attributes.
- **Warned but left as-is**: PostgreSQL array types (`integer[]`) and `INHERITS` — no equivalent is generated.
- **Not detectable**: SQLite auto-increment (`INTEGER PRIMARY KEY`) passes through unchanged when converting _from_ SQLite; MSSQL `TIMESTAMP` (rowversion) is only converted when written as `[TIMESTAMP]`.
