...
Most commands include a `dialect` field (`convert` reports `conversion.from`/`conversion.to` instead, and `redact` and `graph` omit it):
...
A `compression` field (`"gzip"`, `"bzip2"`, `"xz"`, or `"zstd"`) is included only when the input file is compressed.
...
```json
{
  ...
  "issues": [
    {
      "code": "FK_MISSING_PARENT",
      "severity": "error",
      "message": "FK violation in 'orders': references missing row in 'users'",
      "location": {
        "table": "orders",
        "statement_index": 4
      }
...
Each entry's `summary` is the full single-file report (with its own nested `summary`).
...
sql-splitter validate "*.sql" --json | jq '.results[] | select(.passed | not)'
```
