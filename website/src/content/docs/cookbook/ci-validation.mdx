...
```json
{
  ...
  "summary": {
    ...
    "issues": [
      {
        "code": "DDL_MISSING_TABLE",
        "severity": "error",
        "message": "INSERT/COPY references table 'users' with no CREATE TABLE",
        "location": { "table": "users", "statement_index": 1 }
      }
    ],
    "summary": { "errors": 1, "warnings": 0, "info": 0 },
    "checks": { "syntax": "ok", "ddl_dml_consistency": { "failed": 1 } }
  }
}
```
...
if [ -n "$staged_sql" ]; then
  ...
  echo "$staged_sql" | xargs -n1 sql-splitter validate --strict
fi
```
