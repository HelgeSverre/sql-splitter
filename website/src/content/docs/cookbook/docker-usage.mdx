...
Multi-arch images (amd64, arm64) are published to Docker Hub and GHCR on every release:

```bash
# Docker Hub
docker run --rm -v $(pwd):/data helgesverre/sql-splitter analyze /data/dump.sql

# GitHub Container Registry
docker run --rm -v $(pwd):/data ghcr.io/helgesverre/sql-splitter analyze /data/dump.sql
```

Tags: `latest`, exact versions (`1.13.8`), and partial semver (`1.13`, `1`).
...
# Install from GitHub releases (replace VERSION with actual version)
ARG VERSION=1.13.7
RUN apt-get update && apt-get install -y xz-utils && rm -rf /var/lib/apt/lists/*
RUN curl -L https://github.com/helgesverre/sql-splitter/releases/download/v${VERSION}/sql-splitter-x86_64-unknown-linux-gnu.tar.xz | \
    tar -xJ --strip-components=1 -C /usr/local/bin/ \
    sql-splitter-x86_64-unknown-linux-gnu/sql-splitter
...
sql-splitter uses ~20–150 MB memory regardless of file size, so you can set conservative limits:
