...
Enum declarations convert automatically when definitions appear before the tables that use them, as in standard dumps:

- MySQL inline `ENUM()` → PostgreSQL `CREATE TYPE ... AS ENUM` with automatic type creation and inline enum rewriting in DML
- PostgreSQL `CREATE TYPE ... AS ENUM` → MySQL inline `ENUM()`
- Use `--enum-naming` to control the naming scheme for generated PostgreSQL types
- Enum arrays and alterations that cannot preserve every column attribute report a warning
- Unknown PostgreSQL user-defined types remain unchanged because they may be domains, composites, or extension types rather than enums
