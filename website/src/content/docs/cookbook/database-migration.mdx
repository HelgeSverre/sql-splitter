...
Enum conversion between MySQL and PostgreSQL is now fully automatic. `sql-splitter convert` handles both directions:
...
- Unknown enum-like columns fall back to `VARCHAR` with a warning
