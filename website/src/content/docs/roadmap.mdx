...
Generate production-shaped synthetic SQL from a dump, schema, or reusable YAML model:
...
- Editable per-table and per-column YAML rules
- Registered generators and planners for relational and multi-column invariants
- Stable global, table, column, and operator seed streams
- Public library API using the same compiler and engine as the CLI
- Neutral profile evidence reusable by the future `infer` command
