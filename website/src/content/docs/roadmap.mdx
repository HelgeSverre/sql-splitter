...
Accept `.zip` dumps as input across all commands — real-world exports often arrive as `dump.sql.zip`:
...
**Adaptive I/O profiles** — measured on a USB spinning disk, the default
...
```bash
sql-splitter split dump.sql -o tables/ --io-profile auto   # default
sql-splitter split dump.sql -o tables/ --io-profile hdd    # pin it
```
