...
| v1.18.0 | Enum Type Conversion                                                                   | Released |
...
- MySQL inline `ENUM()` → PostgreSQL `CREATE TYPE` with `--enum-naming` flag for deterministic naming
- DML `INSERT` statements rewriiten with inline ENUM values
- `::type` casts stripped from DML statements during conversion
