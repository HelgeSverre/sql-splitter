...
Set the `SQL_SPLITTER_CACHE_DIR` environment variable to override the cache location.

Cache files are named `<hash>.duckdb` where the hash covers the dump file's path, size, and modification time, plus the `--tables` selection and `--dialect`. Each table selection gets its own cache, so a cache built with `--tables users` and a full-dump cache coexist. If the source dump changes, a new cache is created automatically and caches of the old version are cleaned up.
...
| `--progress`     |       | Show import progress                                    | on in a terminal |
...
```bash
# List cached databases (shows source file, contained tables, size)
...
The cache key covers the dump file (path, size, modification time), the `--tables` selection, and `--dialect` — changing any of these triggers a re-import. When an existing cache of the same dump covers a different table set, the re-import says so:

```
Note: existing cache covers {users}; requested {users, orders} - re-importing
```

Cache writes are atomic: an interrupted import never corrupts or removes previously cached databases. To manually clear space, run `--clear-cache` or delete files from the cache directory (see [How It Works](#how-it-works) for paths).
...
# Quick per-table stats without a full import (analyze is faster)
sql-splitter analyze dump.sql --json | jq -r '.tables[] | "\(.name): \(.inserts) inserts"'
...
```bash
sql-splitter query dump.sql --interactive  # then run .tables to list all tables
