...
Schema-qualified table names remain distinct. For a dump with both
`tenant_a.users` and `tenant_b.users`, use the full name in `--tables` to
select one table.

```bash
sql-splitter split postgres_dump.sql --dialect postgres --tables tenant_a.users -o subset/
```
...
```json
{
  ...
  "statistics": {
    "tables_found": 2,
    "statements_processed": 654,
    "bytes_processed": 58023,
    "elapsed_secs": 0.042,
...
```bash
# Extract all tables, merge back a subset
sql-splitter split dump.sql -o all/
sql-splitter merge all/ --tables users,orders,order_items -o subset.sql
```
...
Or for a single compressed output, pipe [`merge`](/commands/merge) through gzip:

```bash
sql-splitter split dump.sql -o tables/
sql-splitter merge tables/ | gzip > combined.sql.gz
```
