...
- `0` - Comparison completed (whether or not differences were found)
- `1` - Error during comparison (missing file, parse error)
- `2` - Invalid CLI arguments

To gate CI on differences, use `--format json` and inspect the summary (see below)—the exit code alone does not signal differences.
...
```bash
# Fail CI if any schema differences exist (exit code is 0 either way,
# so check the JSON summary instead)
sql-splitter diff production.sql staging.sql --schema-only --format json \
  | jq -e '.summary.tables_added + .summary.tables_removed + .summary.tables_modified == 0'
...
# A qualified table name selects that schema only
sql-splitter diff old.sql new.sql --tables tenant_a.users
...
Schema Changes:
  ...
  ~ Table 'users':
      + Column 'phone' TEXT NULL

Data Changes:
  Table 'users': +5 rows, -2 rows, ~10 modified
  Table 'orders': +100 rows

Summary:
  1 tables added, 1 removed, 1 modified
  105 rows added, 2 removed, 10 modified
```
...
### Summary shows changes but no detail

The default output summarizes row changes per table. Use `--verbose` to see sample PK values for the affected rows:
