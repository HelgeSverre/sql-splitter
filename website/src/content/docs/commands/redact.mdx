...
- `0` - Success
- `1` - Error (validation errors, `--strict` mode warnings, file not found, parse failure)
...
For a schema-qualified dump, use the full table name in `--tables` or
`--exclude` to avoid applying the filter to the same table name in another
schema. A rule for one qualified table uses the same final-column form, for
example `tenant_a.users.email`.
...
`shuffle` rules are rejected. They require a two-pass rewrite and are not safe
to apply in the current streaming implementation.
...
rules:
  - column: "*.ssn"
    strategy: "null" # quote it—unquoted null is a YAML null value
  - column: "*.email"
    strategy: hash
  - column: "*.name"
    strategy: fake
...
2. Check strategy spelling: `null`, `hash`, `fake`, `mask`, `constant`, `skip`
