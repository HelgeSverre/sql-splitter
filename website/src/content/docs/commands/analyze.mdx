...
**What counts as an INSERT:** Each `INSERT` statement counts once. A multi-row insert like `INSERT INTO users VALUES (1, 'alice'), (2, 'bob')` counts as 1 INSERT, not 2—the report shows statement counts, not row counts.
...
```
Analyzing SQL file: dump.sql (0.15 MB) [dialect: mysql]

✓ Analysis completed in 25.4ms

Found 4 tables:

Table Name                                    INSERTs  Total Stmts    Size (MB)
────────────────────────────────────────────────────────────────────────────────
order_items                                        12            13         0.09
orders                                              5             6         0.04
products                                            1             2         0.01
users                                               2             3         0.01
────────────────────────────────────────────────────────────────────────────────
...
```json
{
  ...
  "tables": [
    { "name": "users", "inserts": 2, "creates": 1, "statements": 3, "bytes": 12595, "size_mb": 0.01 },
    { "name": "orders", "inserts": 5, "creates": 1, "statements": 6, "bytes": 46284, "size_mb": 0.04 }
  ]
}
```
...
### INSERT count seems wrong

The INSERTs column counts `INSERT` statements, not rows. A multi-row insert like `INSERT INTO t VALUES (1), (2), (3)` counts as 1 INSERT even though it inserts 3 rows. Use [`query`](/commands/query) with `SELECT COUNT(*)` if you need exact row counts.
