...
```bash
# Convert and restore in one step (omit -o to write to stdout)
sql-splitter convert mysql.sql.gz --to postgres | psql "$PG_CONN"

# Convert and load into SQLite
sql-splitter convert pg_dump.sql --to sqlite | sqlite3 dev.db
```
...
| `--enum-naming` |     | Enum type naming strategy: `per-column`, `dedupe`      | `per-column` |
...
| --------------------------- | -------------------------------------------------------- | ---------------------------------------------------------------- |
| MySQL `ENUM('a','b','c')` to PostgreSQL | `CREATE TYPE enum__{table}__{column} AS ENUM ('a','b','c')` + typed column | Semantic-preserving: enum values become a PostgreSQL enum type |
| PostgreSQL `CREATE TYPE ... AS ENUM` to MySQL | Inline `ENUM('a','b','c')` on the column | Semantic-preserving |
| MySQL `ENUM('a','b','c')` to SQLite | `TEXT` | Lossy: enum values are lost |
| MySQL `ENUM('a','b','c')` to MSSQL | `NVARCHAR(255)` | Lossy: enum values are lost |
| PostgreSQL arrays (`INT[]`) | `JSON` (MySQL), `TEXT` (SQLite), `NVARCHAR(MAX)` (MSSQL) | Data format preserved as JSON string                             |

Use `--enum-naming` to control how enum type names are generated for MySQL→PostgreSQL conversions (see [Options](#options)).
...
- `CREATE TYPE` (non-enum) - Custom composite and base types have no equivalent in most dialects. Enum types (`CREATE TYPE ... AS ENUM`) are converted when converting PostgreSQL→MySQL.
