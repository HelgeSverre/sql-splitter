...
| --------------------------- | ------------------------------------------------------------------ | ------------------------------------------------------------------------ |
| MySQL `ENUM('a','b','c')`   | `CREATE TYPE enum__{t}__{c} AS ENUM` (PostgreSQL), `TEXT` (SQLite), `NVARCHAR(255)` (MSSQL) | Semantic-preserving for PostgreSQL → MySQL; `--enum-naming` flag available |
| PostgreSQL `CREATE TYPE`    | Inline `ENUM('a','b','c')` per column (MySQL)                      | Enum types converted from registry; SQLite/MSSQL fall back to string      |
