...
| --------------------------- | ------------------------------------------------------------------ | ------------------------------------------------------------------------ |
| MySQL `ENUM('a','b','c')`   | `CREATE TYPE enum__{t}__{c} AS ENUM` (PostgreSQL), `TEXT` (SQLite), `NVARCHAR(255)` (MSSQL) | Semantic-preserving for MySQL → PostgreSQL; `--enum-naming` flag available |
