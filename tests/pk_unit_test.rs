...
fn test_hash_deterministic() {
    let values1 = [PkValue::Int(42)];
    let values2 = [PkValue::Int(42)];
    assert_eq!(hash_pk_values(&values1), hash_pk_values(&values2));
}

#[test]
fn test_hash_different_values_differ() {
    let v1 = [PkValue::Int(1)];
    let v2 = [PkValue::Int(2)];
    assert_ne!(hash_pk_values(&v1), hash_pk_values(&v2));
}

#[test]
fn test_hash_different_types_differ() {
    let int_val = [PkValue::Int(1)];
    let bigint_val = [PkValue::BigInt(1)];
    let text_val = [PkValue::Text("1".into())];
...
fn test_hash_composite_key() {
    let values = [PkValue::Int(1), PkValue::Text("abc".into())];
    let hash = hash_pk_values(&values);
    assert_ne!(hash, 0);
}
...
fn test_hash_arity_matters() {
    let single = [PkValue::Int(1)];
    let double = [PkValue::Int(1), PkValue::Null];
    assert_ne!(hash_pk_values(&single), hash_pk_values(&double));
}

#[test]
fn test_hash_text_values() {
    let v1 = [PkValue::Text("hello".into())];
    let v2 = [PkValue::Text("world".into())];
    assert_ne!(hash_pk_values(&v1), hash_pk_values(&v2));
}
