...
pub fn run(args: SampleArgs) -> anyhow::Result<()> {
    ...
    // Parse table filters
    let tables_filter = tables.as_deref().map(comma_list);

    let exclude_list: Vec<String> = exclude.as_deref().map(comma_list).unwrap_or_default();
...
fn print_json(
    ...
    let sample_rate =
        crate::transform_common::percent(stats.total_rows_selected, stats.total_rows_seen);

    let tables: Vec<TableSampleJson> = stats
        .table_stats
        .iter()
        .map(|t| {
            let pct = crate::transform_common::percent(t.rows_selected, t.rows_seen);
            TableSampleJson {
                name: t.name.clone(),
