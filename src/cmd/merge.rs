...
mod tests {
    ...
    fn test_discover_sql_files() {
        // Uses real filesystem, skip in unit tests
    }

    #[test]
    fn test_write_header_mysql() {
        let mut buf = Vec::new();
        write_header(&mut buf, SqlDialect::MySql, 5).unwrap();
    ...
    fn test_write_header_postgres() {
        let mut buf = Vec::new();
        write_header(&mut buf, SqlDialect::Postgres, 3).unwrap();
        let output = String::from_utf8(buf).unwrap();
    ...
    fn test_transaction_wrappers() {
        assert!(transaction_start(SqlDialect::MySql).contains("START TRANSACTION"));
        assert!(transaction_start(SqlDialect::Postgres).contains("BEGIN"));
        assert!(transaction_start(SqlDialect::Sqlite).contains("BEGIN TRANSACTION"));

        assert!(transaction_end(SqlDialect::MySql).contains("COMMIT"));
    }
}
